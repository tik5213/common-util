 ...
 * @since 2018-10-20 21:07
 * 创建一个可以滑动的用户展示大图的View对象
 * ImageView 对应的 ID 为 android.R.id.icon
 *
 * 查看大图 使用场景-动态生成一张大图，可以使用此工具类，来生成一个查看器，方便的查看此大图的显示效果
 *
 */
public class DisplayBigPictureUtil {
    public static View getDisplayBigPictureView(Context context){
        return getDisplayBigPictureView(context,true);
    }

    /**
     * 如果不需要支持手机缩放，返回的
     * 其实就是两个横纵交叉的 ScrollView
     *
     * 如果需要支持手势缩放，将返回一个支持手势缩放的自定义控件
     */
    public static View getDisplayBigPictureView(Context context,boolean canZoon){
        if (canZoon){
            return getDisplayBigPictureCanZoomView(context);
        }
        ...
        ScrollView.LayoutParams horizontalParams = new ScrollView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        scrollView.addView(horizontalScrollView,horizontalParams);

        ImageView imageView = new ImageView(context);
        HorizontalScrollView.LayoutParams imageParams = new HorizontalScrollView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
    ...
    /**
     * 展示大图，同时，中间带一个 0.4 alpha 的按钮
     * 中间按钮id android.R.id.button1
     * @param context
     * @param canZoom
     * @param actionBtnText
    ...
    public static View getDisplayBigPictureViewWithActionBtn(Context context, boolean canZoom, String actionBtnText, View.OnClickListener onClickListener){
        ...
        //展示大图
        View displayView = getDisplayBigPictureView(context,canZoom);
    ...
    public static View getDisplayBigPictureCanZoomView(Context context){
        ...
        myBigDrawableImageView.setId(android.R.id.icon);
    ...
    private static class MyBigDrawableImageView extends androidx.appcompat.widget.AppCompatImageView  {
        ...
        public Matrix getInnerMatrix(Matrix matrix) {
            ...
            if (isReady()) {
                //原图大小
                RectF tempSrc = MyBigDrawableImageView.MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable().getIntrinsicHeight());
                //控件大小
                RectF tempDst = MyBigDrawableImageView.MathUtils.rectFTake(0, 0, getWidth(), getHeight());
                //计算fit center矩阵
                matrix.setRectToRect(tempSrc, tempDst, Matrix.ScaleToFit.CENTER);
                //释放临时对象
                MyBigDrawableImageView.MathUtils.rectFGiven(tempDst);
                MyBigDrawableImageView.MathUtils.rectFGiven(tempSrc);
            }
            return matrix;
        }
        ...
        public RectF getImageBound(RectF rectF) {
            ...
            if (!isReady()) {
                ...
                //申请一个空matrix
                Matrix matrix = MyBigDrawableImageView.MathUtils.matrixTake();
                //获取当前总变换矩阵
                ...
                //释放临时matrix
                MyBigDrawableImageView.MathUtils.matrixGiven(matrix);
                return rectF;
            }
        }
        ...
        public boolean canScrollHorizontally(int direction) {
            if (mPinchMode == MyBigDrawableImageView.PINCH_MODE_SCALE) {
                return true;
            }
        ...
        public void outerMatrixTo(Matrix endMatrix, long duration) {
            ...
            if (duration <= 0) {
                ...
                //创建矩阵变化动画
                mScaleAnimator = new MyBigDrawableImageView.ScaleAnimator(mOuterMatrix, endMatrix, duration);
        ...
        public void zoomMaskTo(RectF mask, long duration) {
            ...
            if (duration <= 0 || mMask == null) {
                ...
                //执行mask动画
                mMaskAnimator = new MyBigDrawableImageView.MaskAnimator(mMask, mask, duration);
                mMaskAnimator.start();
            }
        }
        ...
        public interface OuterMatrixChangedListener {
             ...
             *
             * @param myBigDrawableImageView
             *
             * @see #getOuterMatrix(Matrix)
             * @see #getCurrentImageMatrix(Matrix)
             * @see #getImageBound(RectF)
             */
            void onOuterMatrixChanged(MyBigDrawableImageView myBigDrawableImageView);
        }

        /**
         * 所有OuterMatrixChangedListener监听列表
         *
         * @see #addOuterMatrixChangedListener(MyBigDrawableImageView.OuterMatrixChangedListener)
         * @see #removeOuterMatrixChangedListener(MyBigDrawableImageView.OuterMatrixChangedListener)
         */
        private List<MyBigDrawableImageView.OuterMatrixChangedListener> mOuterMatrixChangedListeners;
         ...
         */
        private List<MyBigDrawableImageView.OuterMatrixChangedListener> mOuterMatrixChangedListenersCopy;
         ...
         * @param listener
         */
        public void addOuterMatrixChangedListener(MyBigDrawableImageView.OuterMatrixChangedListener listener) {
            if (listener == null) {
                return;
            }
            ...
            if (mDispatchOuterMatrixChangedLock == 0) {
                if (mOuterMatrixChangedListeners == null) {
                    mOuterMatrixChangedListeners = new ArrayList<MyBigDrawableImageView.OuterMatrixChangedListener>();
                }
                ...
                if (mOuterMatrixChangedListenersCopy == null) {
                    if (mOuterMatrixChangedListeners != null) {
                        mOuterMatrixChangedListenersCopy = new ArrayList<MyBigDrawableImageView.OuterMatrixChangedListener>(mOuterMatrixChangedListeners);
                    } else {
                        mOuterMatrixChangedListenersCopy = new ArrayList<MyBigDrawableImageView.OuterMatrixChangedListener>();
                    }
                }
         ...
         * @param listener
         */
        public void removeOuterMatrixChangedListener(MyBigDrawableImageView.OuterMatrixChangedListener listener) {
            if (listener == null) {
                return;
            }
        ...
        private void dispatchOuterMatrixChanged() {
            ...
            for (MyBigDrawableImageView.OuterMatrixChangedListener listener : mOuterMatrixChangedListeners) {
         ...
         * @see #zoomMaskTo(RectF, long)
         */
        private MyBigDrawableImageView.MaskAnimator mMaskAnimator;
         ...
         * @see #outerMatrixTo(Matrix, long)
         */
        private MyBigDrawableImageView.ScaleAnimator mScaleAnimator;
         ...
         */
        private MyBigDrawableImageView.FlingAnimator mFlingAnimator;
         ...
         * 在onTouchEvent末尾被执行.
         */
        private GestureDetector mGestureDetector = new GestureDetector(MyBigDrawableImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
            ...
            public void onLongPress(MotionEvent e) {
                ...
                if (mOnLongClickListener != null) {
                    mOnLongClickListener.onLongClick(MyBigDrawableImageView.this);
                }
            }
            ...
            public boolean onSingleTapConfirmed(MotionEvent e) {
                ...
                if (mOnClickListener != null) {
                    mOnClickListener.onClick(MyBigDrawableImageView.this);
                }
                return true;
            }
        });
            ...
            if(action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
                ...
                if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                    ...
                    if (mPinchMode == PINCH_MODE_SCROLL) {
                        ...
                        //两个缩放点间的距离
                        float distance = MyBigDrawableImageView.MathUtils.getDistance(event.getX(0), event.getY(0), event.getX(1), event.getY(1));
                        //保存缩放点中点
                        float[] lineCenter = MyBigDrawableImageView.MathUtils.getCenterPoint(event.getX(0), event.getY(0), event.getX(1), event.getY(1));
        ...
        private boolean scrollBy(float xDiff, float yDiff) {
            ...
            //原图方框
            RectF bound = MyBigDrawableImageView.MathUtils.rectFTake();
            getImageBound(bound);
            //控件大小
            ...
            MyBigDrawableImageView.MathUtils.rectFGiven(bound);
            //应用移动变换
        ...
        private void saveScaleContext(float x1, float y1, float x2, float y2) {
            ...
            mScaleBase = MyBigDrawableImageView.MathUtils.getMatrixScale(mOuterMatrix)[0] / MyBigDrawableImageView.MathUtils.getDistance(x1, y1, x2, y2);
            ...
            float[] center = MyBigDrawableImageView.MathUtils.inverseMatrixPoint(MyBigDrawableImageView.MathUtils.getCenterPoint(x1, y1, x2, y2), mOuterMatrix);
        ...
        private void doubleTap(float x, float y) {
            ...
            //获取第一层变换矩阵
            Matrix innerMatrix = MyBigDrawableImageView.MathUtils.matrixTake();
            getInnerMatrix(innerMatrix);
            //当前总的缩放比例
            float innerScale = MyBigDrawableImageView.MathUtils.getMatrixScale(innerMatrix)[0];
            float outerScale = MyBigDrawableImageView.MathUtils.getMatrixScale(mOuterMatrix)[0];
            float currentScale = innerScale * outerScale;
            //控件大小
            float displayWidth = getWidth();
            float displayHeight = getHeight();
            ...
            //开始计算缩放动画的结果矩阵
            Matrix animEnd = MyBigDrawableImageView.MathUtils.matrixTake(mOuterMatrix);
            //计算还需缩放的倍数
            animEnd.postScale(nextScale / currentScale, nextScale / currentScale, x, y);
            //将放大点移动到控件中心
            animEnd.postTranslate(displayWidth / 2f - x, displayHeight / 2f - y);
            //得到放大之后的图片方框
            Matrix testMatrix = MyBigDrawableImageView.MathUtils.matrixTake(innerMatrix);
            testMatrix.postConcat(animEnd);
            RectF testBound = MyBigDrawableImageView.MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable().getIntrinsicHeight());
            ...
            cancelAllAnimator();
            //启动矩阵动画
            mScaleAnimator = new MyBigDrawableImageView.ScaleAnimator(mOuterMatrix, animEnd);
            mScaleAnimator.start();
            //清理临时变量
            MyBigDrawableImageView.MathUtils.rectFGiven(testBound);
            MyBigDrawableImageView.MathUtils.matrixGiven(testMatrix);
            MyBigDrawableImageView.MathUtils.matrixGiven(animEnd);
            MyBigDrawableImageView.MathUtils.matrixGiven(innerMatrix);
        }
        ...
        private void scaleEnd() {
            ...
            //获取图片整体的变换矩阵
            Matrix currentMatrix = MyBigDrawableImageView.MathUtils.matrixTake();
            getCurrentImageMatrix(currentMatrix);
            //整体缩放比例
            float currentScale = MyBigDrawableImageView.MathUtils.getMatrixScale(currentMatrix)[0];
            //第二层缩放比例
            float outerScale = MyBigDrawableImageView.MathUtils.getMatrixScale(mOuterMatrix)[0];
            //控件大小
            float displayWidth = getWidth();
            ...
            //尝试根据缩放点进行缩放修正
            Matrix testMatrix = MyBigDrawableImageView.MathUtils.matrixTake(currentMatrix);
            ...
            MyBigDrawableImageView.MathUtils.rectFGiven(testBound);
            MyBigDrawableImageView.MathUtils.matrixGiven(testMatrix);
            MyBigDrawableImageView.MathUtils.matrixGiven(currentMatrix);
        }
        ...
        private void fling(float vx, float vy) {
            ...
            mFlingAnimator = new MyBigDrawableImageView.FlingAnimator(vx / 60f, vy / 60f);
        ...
        private class FlingAnimator extends ValueAnimator implements ValueAnimator.AnimatorUpdateListener {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                //速度太小或者不能移动了就结束
                if (!result || MyBigDrawableImageView.MathUtils.getDistance(0, 0, mVector[0], mVector[1]) < 1f) {
                    animation.cancel();
                }
            }
        }
         ...
         * 矩阵对象池
         */
        private static class MatrixPool extends MyBigDrawableImageView.ObjectsPool<Matrix> {
         ...
         * 矩形对象池
         */
        private static class RectFPool extends MyBigDrawableImageView.ObjectsPool<RectF> {
        ...
        public static class MathUtils {
             ...
             * 矩阵对象池
             */
            private static MyBigDrawableImageView.MatrixPool mMatrixPool = new MyBigDrawableImageView.MatrixPool(16);
             ...
             * 矩形对象池
             */
            private static MyBigDrawableImageView.RectFPool mRectFPool = new MyBigDrawableImageView.RectFPool(16);
